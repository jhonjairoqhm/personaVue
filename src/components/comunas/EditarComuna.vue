...
export default {
  ...
  async updateComuna() {
    const res = await axios.put(`http://127.0.0.1:8000/api/comunas/${this.comuna_codi}`, this.comuna);
...
mounted() {
  this.comuna_codi = this.$route.params.id;
  axios.get(`http://127.0.0.1:8000/api/comunas/${this.comuna_codi}`)
    .then(response => {
